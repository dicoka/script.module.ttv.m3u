...
def get_chan_url (ch, i=0):
    ...
    data = GET(
        "http://{0}/v3/translation_stream.php?session={1}&channel_id={2}&typeresult=json".format(API_MIRROR, session,
                                                                                                 chID))
    try:
...
if tryStringToInt(jdata['support']) == 0:
    msg = "Текущая версия приложения (%s) не поддерживается. Последняя версия %s " % (TTV_VERSION, jdata['last_version'].encode('utf-8'))
    showNotification(msg, xbmcgui.NOTIFICATION_ERROR)
    sys.exit(0)
...
data = GET(
    ...
    cookie=['PHPSESSID=%s' % session], trys=10)
